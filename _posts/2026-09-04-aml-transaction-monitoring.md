...
*The interactive version of the dashboard is available **[here.](https://public.tableau.com/app/profile/christine.dowling/viz/AMLProgramEffectiveness/AMLProgramEffectiveness)** Capacity and operating-model can be toggled to see the impact on the metrics.*
...
That last column is unique and valuable. A **typology** is a named pattern of criminal behaviour, and the dataset labels 17 distinct suspicious ones alongside 11 normal ones. In plain terms, a few of the suspicious ones are:
...
Therefore, in order to make use of this data I need to derive new measurements that describe an account's (client's) *behaviour over time* - also known as **feature engineering**. Rather than using Pandas to complete this task (given the size of the dataset), I built these using DuckDB, which allowed me to run  SQL-based queries within a notebook without loading the whole file into memory, computing rolling time windows across millions of rows far faster than standard Python tools.
...
Here I sampled **whole accounts**: every account involved in any laundering, plus a random selection of clean ones, keeping all of their transactions. That preserved behaviour and left **2,461,050 transactions containing all 9,873 laundering cases.**

The side effect is that laundering now looks about four times more common than it really is. I measured that enrichment precisely (**3.874×**) and corrected for it whenever reporting results.
...
**Thresholds are set per payment type.** My first attempt used one global amount threshold and the rule never flagged activity. The reason for this was that different payment types had vastly different caps, making a single threshold useless.
...
**Two rules are close to worthless.** I measured **marginal value** by looking at how many unique detections would be lost if a rule were switched off entirely. Structuring contributed **4 unique detections** for 13,540 alerts and 940 analyst-days. Pass-through contributed **2**, for 2,949 alerts. Between them: 16,489 alerts, 1,145 analyst-days, and only 6 unique detections.  

**Coverage is uneven.** The rule engine sees some behaviour almost perfectly and other behaviour barely at all:
...
While the first two findings might look like easy targets for cutting costs, removing or altering these rules should be a business decision which takes into account the real and full cost (i.e. the cost of missed laundering transactions).

**The gap in coverage is the case for adding a model.** Layered structures are deliberately designed to defeat simple rules and where a machine learning model could add value in finding non-linear connections.

___

# 05. The Machine Learning Model <a name="the-model"></a>

Taking some lessons from the Fraud Detection project, I trained an **XGBoost** classifier since it is appropriate for the messy, non-linear interactions in behavioural data, and it handles missing data. I also used Bayesian optimization (Optuna) to tune hyperparameters rather than using brute force (e.g. GridSearch). Two points differed from the previous project:
...
Interestingly, payment amount contributed under 2%, which tells us that the *pattern of movement* is more important than the size of any single payment.

### Thresholding <a name="thresholding"></a>

Similar to the Fraud Detection project, I also implemented thresholding since this has a direct impact on whether a something is considered an alert or not. To prevent leakage I swept **out-of-fold** predictions from the training period, reusing the same rolling-origin folds, so each score came from a model that never saw that data. I also only focused on two possible thresholding strategies: 1) Best F-1, and 2) Capacity. The former treats false positives and false negatives as equally costly, while the latter is constrained by resources (a more in-depth review of these options can be found in the Fraud Detection project).  
...
If we focus on maximizing recall (that is, accurately catching laundering activity), the thresholding choice is Capacity. Additionally, this is more realistic and mirrors how an AML team might actually function. 

NOTE: To arrive at the thresholds, I assumed a hypothetical dedicated team comprised of 5 analysts working only with the model. These numbers are intended to demonstrate the mechanism and would need to be reconciled with the remainder of the analysis below.
___
...
**B — Rules plus triage.** Exactly the same alerts, sorted by model score so the most likely productive alerts are moved earlier in the queue. **This detects nothing new,** and its entire value is ordering (nothing is added/removed). Each account-day alert inherits the **highest** model score among its transactions, and the queue is sorted by that score.
...
Two things stand out. At a quarter of capacity, ranking alone more than triples detection for identical cost. And at full capacity A and B are **identical** — because if you work every alert, the order is irrelevant. 
...
The blanks are also a finding. Scenarios A and B cannot reach 30% at any cost — **26.4% is the structural ceiling of the ruleset**, and no amount of headcount changes it.
...
# 07. Limitations <a name="limitations"></a>
...
**Triage and detection are different products.** Triage makes the existing program cheaper and is comparatively easy to approve. Hybrid detection makes it *better* but requires a bank to defend a model-driven alert to regulators. 
